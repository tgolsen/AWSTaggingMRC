...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM taggables WHERE Type = 'Certificate'"); // AND helpful_string IS NULL");
    $stmt->execute();
...
//        die;
        ...
        $arn = "arn:aws:acm:"+ $row['region'] + ":929875250258:certificate/" + $row['Identifier'];

        if (!$arn) {
            echo "No arn found for Snapshot row ID: {$row['Identifier']}\n";
            continue;
        }
...
//        echo "Processing Snapshot ID: {$snapshotId}\n";
        ...
        $command = "aws --profile=mrc ec2 describe-snapshots --snapshot-ids {$arn} --query 'Snapshots[0].Description' --output text 2> /dev/null";
